...
1. 遠端分支
	...
	顧名思義，遠端分支就是在遠端儲存庫中的分支，如此而已。遠端分支無法在本地存取，必須經由特殊的指令連線操作，且所有操作皆須得到遠端儲存庫的許可。

2. 遠端追蹤分支

	當你執行 `git clone` 複製一個遠端容器回來之後，所有遠端分支會被下載回來，並且相對應的建立起同名的 **遠端追蹤分支**。

	我們以複製 jQuery 在 GitHub 上的專案為例，透過 `git clone https://github.com/jquery/jquery.git` 下載回來後，執行 `git branch -a` 指令，顯示出所有「本地分支」與「遠端追蹤分支」。「遠端追蹤分支」就是如下圖**紅字**的部分：

	![image](https://f.cloud.github.com/assets/88981/1405811/8c59cc46-3d3f-11e3-9919-c71b5ccb87ca.png)

	「遠端追蹤分支」主要就是用來跟遠端分支做對應，你不應該直接在這些分支上建立版本 (雖然你還是可以這麼做，但強烈不建議亂搞)，而是把這些「遠端追蹤分支」視為是一種｢唯讀｣的分支。

3. 本地分支

	在透過 `git branch` 指令執行時所顯示的分支，就是所謂的「本地分支」，這些分支存在於本地端。
	其中有些本地分支可設定追蹤遠端追蹤分支，它們又稱作**追蹤分支**，追蹤的對象又稱作**上游**，追蹤分支和上游分支在許多操作上有緊密的關聯，而 Git 也會時常在操作追蹤分支時提示其上游的狀態。
	
	另一些沒有追蹤的分支依使用方式常稱作 **主題分支** (Topic Branch) 、 **開發分支** (Development Branch) 、 **特性分支** (Feature Branch) 等等，它們一般不會推送到遠端儲存庫，主要用於細部的開發，且多半會合併到追蹤分支乃至其上游。
